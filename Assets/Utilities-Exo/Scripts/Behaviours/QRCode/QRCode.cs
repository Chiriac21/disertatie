...
namespace QRTracking
{
    ...
    public class QRCode : MonoBehaviour
    {
        ...
        public bool objInstantiated = false;

        private GameObject rosy;
        private GameObject currentObj;
        private float y_obj;
        private float z_obj;
        private float y_rosy;

        [SerializeField] private GameObject rosyPrefab;
        [SerializeField] private GameObject WLPrefab;
        ...
        void Start()
        {
            ...
            currentObj = GameObject.Find("QRCode(Clone)");
        ...
        void Update()
        {
            y_rosy = currentObj.transform.localRotation.eulerAngles.y + currentObj.transform.localRotation.eulerAngles.z;

            if (objInstantiated == false && qrCode != null && currentObj.transform.position != new Vector3(0, 0, 0))
            {
                count++;
                rosy = Instantiate(rosyPrefab, new Vector3(currentObj.transform.position.x, currentObj.transform.position.y,
                    currentObj.transform.position.z), Quaternion.identity);
                FindObjectOfType<DialogInit>().IsDetected = true;
                GameObject.FindGameObjectWithTag("MainMenu").SetActive(false);
                FindObjectOfType<InstantiateRosy>().welcomePanel.SetActive(false);
                objInstantiated = true;
            }

            rosy.transform.position = currentObj.transform.position;
            rosy.transform.rotation = Quaternion.Euler(0, y_rosy, 0);
